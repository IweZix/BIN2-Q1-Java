...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        try{
            FileReader fileReader = new FileReader("./ressources/streamingvf.csv");
    ...
    private static String firstLine() {
        return supplier.get().findFirst().isPresent() ?
                supplier.get().findFirst().get() :
                "None";
    }
    ...
    private static List<String> filteredLastnames() {
        ...
        List<String> list = supplier.get()
                .map(s -> new Employee(s).getLastname())
                .filter(predicate)
                .toList();
        return list;
    }
    ...
    private static String longLastName() {
        Predicate<String> predicate = s -> s.length() > 14;
        List<String> list = supplier.get()
                .map(s -> new Employee(s).getLastname())
                .filter(predicate)
                .toList();
    ...
    private static Map<Boolean, List<Integer>> timeDistrubution() {
        return supplier.get()
                .map(s -> new Employee(s))
                .collect(Collectors.groupingBy(Employee::isFullTime, Collectors.mapping(Employee::getId, Collectors.toList())));
    }
