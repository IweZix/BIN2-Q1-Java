...
public class ExerciceGroupingBy {
    ...
    private void groupBy1() {
        ...
        Map<Trader, List<Transaction>> map =
    ...
    private void groupBy3() {
        ...
        Map<TransactionsLevel, List<Transaction>> map =
                transactions.stream().collect(
                        groupingBy( trans -> {
                            if (trans.getValue() >= 1000)
                                return TransactionsLevel.VERY_HI;
                            else if (trans.getValue() >= 800 && trans.getValue() < 1000)
                                return TransactionsLevel.HI;
                            else if (trans.getValue() >= 600 && trans.getValue() < 800)
