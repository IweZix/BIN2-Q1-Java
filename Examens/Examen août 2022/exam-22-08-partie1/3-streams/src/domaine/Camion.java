...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets.stream()
                .filter(t -> t.nbCaisses() < nbMaxCaisses)
                .sorted(Comparator.comparing(Trajet::getVilleDepart)
                        .thenComparing(Trajet::getVilleArrivee))
                .toList();
    }
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets.stream()
                ...
                .filter(t -> t.getVilleDepart().equals(villeDepart))
                .filter(t -> t.getVilleArrivee().equals(villeDArrivee))
                .map(Trajet::getDate)
