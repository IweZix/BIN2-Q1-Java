...
public class Trajet implements Iterable<Caisse>{
    ...
    public Map<LocalDate, Long> nombreDeCaissesParDateLimite() {
        for (Caisse caiss : caisses) {
            System.out.println(caiss.getDateLimite()    );
        }
        return caisses.stream()
                .sorted(Comparator.comparing(Caisse::getDateLimite))
                .collect(Collectors.groupingBy(Caisse::getDateLimite, Collectors.counting()));
    }
}
